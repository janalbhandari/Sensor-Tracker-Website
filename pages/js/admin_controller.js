...
admin_page.controller('admin_page_controller',['$scope','$http',function($scope,$http){
		...
		$http.get('/user_rec').success(function(response){
			...
			for(i=0;i<response.length;i++)
			{
				...
				active_phone +=response[i].phone.length;
				active_array[i]=active_fitbit+active_car+active_phone;
			...
			console.log("phone---"+active_phone);
		...
		$http.get('/inactive_sensors_data').success(function(response)
		{
			...
			for(i=0;i<response.length;i++)
			{
				...
				{
					inactive_phone=inactive_phone+1;	
				}
			...
			console.log("inact-phone---"+inactive_phone);
