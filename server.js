...
var db = mongojs('DevClient:DevClientPass@ds149207.mlab.com:49207/cloudiaas', ['users']); 
//establishing connection to remote database , username : DevClient , password : DevClientPass, database uri : ds149207.mlab.com:49207/cloudiaas
...
app.post('/stop_fitbit/:response/:date',function(req,res){  
    ...
    res.send("Fitbit disconnected");
});



//-----------------phone sensor addition

app.post('/new_phone/:response/:date',function(req,res){
    console.log("For new phone:" + req.params.response);
    console.log("For new phone:" + req.params.date);
    db.users.update(
       { "email": req.params.response },
       { $addToSet: { phone: {
            start_date:req.params.date,
...
//--------------------- to remove phone sensor 


app.post('/stop_phone/:response/:date',function(req,res){  
    var fitbit_array;
    var start_date;
    console.log("For phone:" + req.params.response);
    console.log("For end_date phone:" + req.params.date);


    db.users.find({"email":req.params.response},function(err,docs){
        console.log(docs[0].phone[0].start_date);
        db.sensor_usage.insert({
                "email":req.params.response,
                "sensor_name":"phone",
                "start_date":docs[0].phone[0].start_date,
                "end_date":req.params.date       
        });
        db.users.update({"email":req.params.response}, { $pop: { "phone": -1 } } );
        
    });

    res.send("phone disconnected");
});

//-------------- car sensor addition

app.post('/new_car/:response/:date',function(req,res){
    console.log("For new car:" + req.params.response);
    console.log("For new car:" + req.params.date);
    db.users.update(
       { "email": req.params.response },
       { $addToSet: { car: {
            start_date:req.params.date,
...
app.get('/user_total_Sensor_query/:email',function(req,res){
 		...
 		db.users.find({"email":req.params.email},function(err,docs){
 			...
 			var phone_length=docs[0].phone.length;
 			db.sensor_usage.find({"email":req.params.email},function(err,docs){
 				console.log("inactive total ------"+docs.length);
 				...
 				for(i=0;i<try_length;i++)
 				{
 					...
 					else if(docs[i].sensor_name=="phone")
 					{
 						phone_length=phone_length+1;	
 					}
 				...
 				res.send([fitbit_length,car_length,phone_length]);
 			});
 			//res.json(docs);
 		});
